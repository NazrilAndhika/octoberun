...
<aside id="admin-sidebar" class="w-64 bg-[#0b4d75] text-white h-screen fixed left-0 top-0 z-50 shadow-xl flex flex-col transition-transform duration-300 -translate-x-full md:translate-x-0">
    ...
    <nav class="mt-6 px-4 space-y-1 flex-1 overflow-y-auto">
        ...
        <!-- Menu Paket Tiket -->
        <a href="{{ route('admin.ticket_packages.index') }}" class="flex items-center gap-3 px-4 py-3 rounded-lg font-bold transition {{ request()->routeIs('admin.ticket_packages.*') ? 'bg-white/10 text-white' : 'text-blue-100 hover:bg-white/10 hover:text-white' }}">
        ...
        <!-- Menu FAQ (Dipindah ke Konten Website) -->
        {{-- 
